...
Checks node reservation status and updates a status file
"""
...
def update_motd(node_data):
    """Update MOTD with node status (writes to C:\ReBM\motd.txt)"""
    try:
        motd = []
        motd.append("=" * 50)
        ...
        Path(MOTD_PATH).parent.mkdir(parents=True, exist_ok=True)
